...
import cd.test.Interpreter.MethodInterp.EarlyReturnException;
import cd.util.FileUtil;

/**
 * An interpreter for the Javali IR.  It requires that the IR
 * be fully semantically analyzed -- in particular, that symbols
 * be assigned to Var and Field nodes. 
 * 
 * It can interpret either the AST used in CD1 or the CFG from
 * CD2.  It detects infinite loops and also tracks how many
 * operations of each kind were performed.
 */
public class Interpreter {
	...
	public void execute() {
		invokeMethod("Main", "main", new JlObject("Main"), Collections.<JlValue>emptyList());
	}
	...
	private void increment(Object operator) {
		...
		if (current == null) opCounts.put(operator, 1);
		else opCounts.put(operator, current + 1);
	}
	...
	abstract class JlValue {
		...
		public JlValue(String s) { typeName = s; }
		public boolean asBoolean() { throw new StaticError("Not a bool"); }
		public int asInt() { throw new StaticError("Not an int"); }
		public float asFloat() { throw new StaticError("Not an float"); }
		public JlReference asRef() { throw new StaticError("Not a reference"); }
		public JlObject asObject() { throw new StaticError("Not an object"); }
	
		// Binary Operations
		// Arithmetic
		public JlValue times(JlValue value) { throw new StaticError("* Operation not supported"); } 
		public JlValue div(JlValue value) { throw new StaticError("/ Operation not supported"); }
		public JlValue add(JlValue value) { throw new StaticError("+ Operation not supported"); }
		public JlValue subtract(JlValue value) { throw new StaticError("- Operation not supported"); }
		public JlValue mod(JlValue value) { throw new StaticError("% Operation not supported"); }
		// Boolean
		public JlValue or(JlValue value) { throw new StaticError("&& Operation not supported"); }
		public JlValue and(JlValue value) { throw new StaticError("|| Operation not supported"); }
		// Comparison
		public JlValue less(JlValue value) { throw new StaticError("< Operation not supported"); }
		public JlValue lessOrEqual(JlValue value) { throw new StaticError("<= Operation not supported"); }
		public JlValue greater(JlValue value) { throw new StaticError("> Operation not supported"); }
		public JlValue greaterOrEqual(JlValue value) { throw new StaticError(">= Operation not supported"); }
		// Unary Operation
		public JlValue plus() { throw new StaticError("+ Operation not supported"); }
		public JlValue minus() { throw new StaticError("- Operation not supported"); }
		public JlValue not() { throw new StaticError("not Operation not supported"); }
	}
	...
	class JlObject extends JlReference {			
		...
		@Override public boolean canBeCastTo(String typeName) {
			...
			while (!currentType.equals("Object")) { 
				...
				if (superTypes.contains(currentType)) {
					throw new StaticError("Circular inheritance: "+currentType);
				}
	...
	class JlArray extends JlReference {
		...
		public JlArray(String s, int size) {
			...
			if (size < 0) {
				throw new DynamicError("Invalid array size: " + size, DynamicError.INVALID_ARRAY_SIZE);
			}
		...
		public JlArray asArray() { return this; }
		...
		public JlValue deref(int idx) { 
			...
			try {
				...
				throw new DynamicError("Array index out of bounds " + idx, DynamicError.INVALID_ARRAY_BOUNDS);
			}
	...
	class JlNull extends JlReference {
		...
		public JlObject asObject() { 
			throw new DynamicError("Null pointer dereferenced", DynamicError.NULL_POINTER);
		}
		...
		@Override 
		public boolean canBeCastTo(String typeName) { return true; }
	...
	class JlUninitialized extends JlValue {
		...
		public JlUninitialized() { super("uninitialized"); }
		
		@Override 
		public boolean asBoolean() {
			throw new DynamicError("Uninitialized value!", DynamicError.UNINITIALIZED_VALUE);
		}
		...
		@Override
		public String toString() { return "(uninit)"; }
	...
	class JlFloat extends JlValue {
		...
		@Override
		public JlValue plus() { return new JlFloat(value); }
		
		@Override
		public JlValue minus()  { return new JlFloat(-value); }
	...
	class JlInt extends JlValue {
		...
		@Override
		public JlValue plus() { return new JlInt(value); }
		
		@Override
		public JlValue minus()  { return new JlInt(-value); }
	...
	class JlBoolean extends JlValue {
		...
		@Override
		public JlValue not() { return new JlBoolean(!value); };
	...
	public void step() {
		...
		if (++steps > MAX_STEPS) { 
			throw new DynamicError("Uninitialized value!", DynamicError.INFINITE_LOOP);
		}
	...
	class ExprInterp extends AstVisitor<JlValue, StackFrame> {
		...
		public JlValue binaryOp(BinaryOp ast, StackFrame arg) {
			...
			try {
				...
				switch (ast.operator) {
				case B_TIMES: return left.times(right);
				case B_DIV: return left.div(right);
				case B_MOD: return left.mod(right);
				case B_PLUS: return left.add(right);
				case B_MINUS: return left.subtract(right);
				case B_AND: return left.and(right);
				case B_OR: return left.or(right);
				case B_EQUAL: return v(left.equals(right));
				case B_NOT_EQUAL: return v(!left.equals(right));
				case B_LESS_THAN: return left.less(right);
				case B_LESS_OR_EQUAL: return left.lessOrEqual(right);
				case B_GREATER_THAN: return left.greater(right);
				case B_GREATER_OR_EQUAL: return left.greaterOrEqual(right);
				}
			
				throw new DynamicError("Unhandled binary operator", DynamicError.INTERNAL_ERROR);
				
			} catch (ArithmeticException e) {
				throw new DynamicError("Division by zero", DynamicError.DIVISION_BY_ZERO);
			}
		}
		...
		public JlValue cast(Cast ast, StackFrame arg) {
			...
			throw new DynamicError("Cast failure: cannot cast "+ ref.typeName+" to "+ast.typeName, DynamicError.INVALID_DOWNCAST);
		...
		public JlValue unaryOp(UnaryOp ast, StackFrame arg) {
			...
			switch (ast.operator) {
				case U_PLUS: return val.plus();
				case U_MINUS: return val.minus();
				case U_BOOL_NOT: return val.not();
			}
			
			throw new DynamicError("Unhandled unary operator "+ast.operator, DynamicError.INTERNAL_ERROR);
		}
		...
		public JlValue var(Var ast, StackFrame arg) {
			...
			if (ast.sym == null) {
				throw new DynamicError("Var node with null symbol", DynamicError.INTERNAL_ERROR);
			}
			...
			throw new DynamicError("Unhandled VariableSymbol kind: "+ast.sym.kind, DynamicError.INTERNAL_ERROR);
	...
	public MethodDecl findMethodDecl(String origTypeName, final String methodName) {
		...
		while (!typeName.equals("Object")) {
			...
			if (result.size() > 1) {
				throw new StaticError(result.size() + " implementations of method " + methodName + " found in type " + typeName);
			}
			
			typeName = cd.superClass;
		}
		
		throw new StaticError("No method "+methodName+" in type "+origTypeName);
	...
	public JlValue invokeMethod(final String typeName, final String methodName, final JlObject rcvr, final List<JlValue> arguments) {
