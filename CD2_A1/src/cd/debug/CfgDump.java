	...
	private void dumpBlocks(boolean dumpDominators, MethodDecl mdecl, String m) {
		for (BasicBlock blk : mdecl.cfg.allBlocks) {
			...
			if (!dumpDominators || true) {
				...
				{
					for (Entry<VariableSymbol, Phi> entry : blk.phis.entrySet()) {
						blklbl.append("|");
						blklbl.append(entry.getKey().name).append(": ").append(entry.getValue());
					}	
				}
				for (Ast instr : blk.instructions) {
					blklbl.append("|").append(AstOneLine.toString(instr));
				}
